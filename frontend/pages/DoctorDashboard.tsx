          ...
          <div className="flex-1 overflow-y-auto overscroll-contain">
            {conversations.length > 0 ? conversations.map(convo => (
              <button key={convo.patient.id} onClick={() => handleSelectConversation(convo.patient)} className={`w-full text-left p-4 border-b border-slate-100 hover:bg-slate-50 ${selectedPatient?.id === convo.patient.id ? 'bg-sky-50' : ''}`}>
                ...
                <p className="text-sm text-slate-500 truncate" dir="auto">{convo.lastMessage.text}</p>
              </button>
            ...
            <>
              ...
              <div className="p-3 sm:p-4 border-t bg-slate-50 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} placeholder="Type your message..." dir="auto" className="flex-1 min-w-0 px-4 py-2 border text-slate-800 rounded-full focus:outline-none focus:ring-2 focus:ring-sky-500" disabled={isLoading}/>
...
const NurseMessagingView: React.FC<{ doctor: Doctor }> = ({ doctor }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md flex flex-col overflow-hidden" style={{ height: 'calc(100dvh - 10rem)', minHeight: '400px' }}>
      <div className="flex-1 flex relative overflow-hidden">
        {/* Conversation List */}
        <div className={`absolute inset-0 sm:relative sm:inset-auto sm:w-1/3 flex-shrink-0 border-r border-slate-200 flex flex-col bg-white z-10 transition-transform duration-300 ease-in-out ${selectedNurseConvo ? '-translate-x-full sm:translate-x-0' : 'translate-x-0'}`}>
          <div className="p-4 border-b flex-shrink-0">
            <h3 className="text-xl font-semibold text-slate-800">Nurse Messages</h3>
          </div>
          <div className="flex-1 overflow-y-auto overscroll-contain">
            {conversations.length > 0 ? conversations.map(item => (
              <button key={item.conversation.conversation_id} onClick={() => setSelectedNurseConvo(item.conversation)} className={`w-full text-left p-4 border-b border-slate-100 hover:bg-slate-50 ${selectedNurseConvo?.conversation_id === item.conversation.conversation_id ? 'bg-sky-50' : ''}`}>
                <div className="flex justify-between items-center">
                  <p className="font-semibold text-slate-800">{item.conversation.nurse_name}</p>
                ...
                <p className="text-sm text-slate-500 truncate" dir="auto">{item.conversation.last_message || 'No messages yet'}</p>
              </button>
            )) : <p className="p-4 text-slate-500">No nurse conversations yet.</p>}
          </div>
        </div>
        {/* Chat View */}
        <div className={`absolute inset-0 sm:relative sm:inset-auto sm:w-2/3 flex-shrink-0 flex flex-col bg-white transition-transform duration-300 ease-in-out ${selectedNurseConvo ? 'translate-x-0' : 'translate-x-full sm:translate-x-0'}`}>
          {selectedNurseConvo ? (
            <>
              <div className="p-4 border-b flex items-center flex-shrink-0">
                <button onClick={() => setSelectedNurseConvo(null)} className="sm:hidden mr-4 p-2 rounded-full hover:bg-slate-100">&larr;</button>
                <h3 className="text-xl font-semibold text-slate-800 truncate">Chat with Nurse {selectedNurseConvo.nurse_name}</h3>
              </div>
              <div className="flex-1 p-4 sm:p-6 overflow-y-auto overscroll-contain space-y-4">
                {messages.map((msg, idx) => {
                  const msgDate = new Date(msg.timestamp);
